...
@dataclass
class TextRecognizerPARSeqLargeV41DynwConfig:
    """Trial config for the dynamic-width fine-tuned PARSeq checkpoint.

    Same architecture as parseq-large-v4_1. Weights live in a local
    HF-format directory produced by ``scripts/register_hugging_face_hub.py``
    (with the push_to_hub step disabled) from the dynw checkpoint, so
    from_pretrained loads them without any download. The checkpoint was
    fine-tuned with the dynw recipe (width bucketing + random trailing
    margin) so it is robust to narrow canvases, which unlocks accurate
    dynamic-width batching at inference time.
    """

    # Local HF-format directory (save_pretrained output), loaded by
    # from_pretrained without contacting the Hub.
    hf_hub_repo: str = str(ROOT_DIR + "/local_models/parseq-large-v4_1-dynw")
