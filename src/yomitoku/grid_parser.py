...
def _get_col_headers_from_grid(grid, is_header_row):
    n_cols = len(grid[0])
    n_rows = len(grid)
...
def parse_grid_from_bottom_up(cells, clustered_nodes):
    ...
    col_headers = _get_col_headers_from_grid(grid, is_header_row)
    data = [raw for i, raw in enumerate(grid) if not is_header_row[i]]
