...
| ocr_vis | `ndarray` \| `None` | OCR モジュールの可視化（`visualizer=True` の時のみ） |
| layout_vis | `ndarray` \| `None` | Layout Analyzer モジュールの可視化（`visualizer=True` の時のみ） |
...
| ocr_vis | `ndarray` \| `None` | AI-OCR モジュールの可視化（`visualizer=True`の時のみ） |
