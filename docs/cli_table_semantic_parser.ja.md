...
セルIDは `r{行}c{列}` 形式の位置ベースIDです。行・列はテーブル内のセル座標のクラスタリングから導出されるため、他のセルの検出増減や数pxの座標ずれの影響を受けにくく、`--vis_id` で画像上の位置と対応付けられます。同一位置に複数セルが検出された場合は `_2` などのサフィックスで区別されます。なお実行間の厳密な照合が必要な場合は、IDではなく座標ベースの照合（テンプレートの `match_policy: bbox`）を使用してください。
