...
## 解析結果の可視化
セルIDを含めて可視化した例です（CLIでは `yomitoku_table ${path_data} -o results -v --vis_id` で出力できます）。
...
たとえば、サンプル帳票では次のような要素が `kv_items` に該当します。（可視化結果では緑色の矢印でキーから値への関係が描画されます。）
...
### Cells
各テーブル内のセルは`cells`と呼ばれるスキーマに情報が保持されます。これらにはテーブルごとに固有識別子`id`が付与されます。セルIDはセルの位置に基づく `r{行番号}c{列番号}` 形式（`r0c0`など）です。
...
テーブル情報の指定の方法はIDによる指定と位置による指定の2パターンが用意されています。テーブル情報を取得したあとセルの情報を指定したり、意味情報を利用して特定のセルの情報を抽出可能です。
...
> 備考
> セルがネスト構造になっている場合に複数のKeyを持つ可能性があるため`key` のみ複数形のリストになっています。上記の例では親ヘッダーの`申込者情報`と`団体名`の2つのセルがKeyに含まれています。
> 検索したときに複数のKey-Valueペアがヒットする場合があるため、戻り値はリストで返却されます。
...
キーがネスト構造になっている場合、キーセルの入れ子構造を保った階層dictになります。同一キーに複数の値が存在する場合は`separator`（デフォルトは改行）で結合されます。

**引数**

* `separator: str = "\n"`: 同一キーの複数valueを結合するセパレータ

**戻り値**

* `dict`: 値は文字列、または入れ子の辞書（子キーを持つ場合）。同じテキストの別キーが並ぶ場合はリストになります。
...
#### results.to_json
書類全体の解析結果をJSONで保存します。出力先のディレクトリが存在しない場合は事前に作成してください。
...
load_json は 「解析結果を保存したJSONを読み込み、TableSemanticParserSchema（= results）として復元する」ためのユーティリティです。AIによる再処理を行わずに後段の抽出処理の再実行が可能です。
...
`kv_items` はキーセルの入れ子構造を保った階層dictになります。同一キーに複数の値が存在する場合は `separator`（デフォルトは改行）で結合されます。
...
テーブル内に含まれる`kv_items`を一つのJSONにまとめエクスポートします。`results.to_dict`と同様に、キーセルの入れ子構造を保った階層dictとして出力されます。同一キーに複数の値が存在する場合は`separator`（デフォルトは改行）で結合されます。
...
* `out/grid_g0.csv` のように grid_id が付与されます。テーブル内に複数のgridsが含まれる場合は`out/grid_g1.csv`のように接尾字が変化します。
